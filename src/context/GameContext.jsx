...
export function GameProvider({ children }) {
  ...
  const sanitizeAvatar = (avatar) => {
    if (avatar && avatar.icon) return avatar;
    return AVATARS[0];
  };
  ...
  const sanitizePlayers = (rawPlayers) => {
    if (!Array.isArray(rawPlayers) || rawPlayers.length === 0) return players;
    ...
    return rawPlayers.map((p, idx) => {
      ...
      return {
        ...
        name: (p?.name && String(p.name).trim() !== '') ? String(p.name).trim() : defaultNames[idx % defaultNames.length],
        avatar: sanitizeAvatar(p?.avatar || AVATARS[idx % AVATARS.length]),
        lives: typeof p?.lives === 'number' ? p.lives : 1,
        score: typeof p?.score === 'number' ? p.score : 0,
        hand: Array.isArray(p?.hand) && p.hand.length > 0 ? p.hand : [card1, card2],
  ...
  const startGame = (customPlayerConfigs = [], roomCode = null) => {
    ...
    const sanitized = sanitizePlayers(customPlayerConfigs);
  ...
  const handleTimeOut = () => {
    addLog(`⏰ ORTAK SÜRE BİTTİ! Bomba ${players[turnIndex]?.name || 'Oyuncu'}'in elinde patladı!`);
  ...
  const passTurn = (targetIdx = null) => {
    ...
    let attempts = 0;
    while (players[nextIdx] && players[nextIdx].lives <= 0 && attempts < total) {
    ...
    addLog(`👉 SIRA: ${players[nextIdx]?.name || 'OYUNCU'}'TA!`);
  ...
  const answerQuestion = (optionIndex) => {
    ...
    if (optionIndex === currentQuestion?.correct) {
      ...
      addLog(`✅ ${players[turnIndex]?.name || 'Oyuncu'} doğru bildi! (+10 Puan)`);
      passTurn();
    } else {
  ...
  const triggerExplosionForPlayer = (playerIdx) => {
    ...
    const victim = players[playerIdx] || { name: 'Oyuncu' };
  ...
  const playCard = (card, targetPlayer = null) => {
    ...
    const lastPlayed = { ...card, player: players[turnIndex]?.name || 'Oyuncu' };
    ...
    switch (card.id) {
      case 'PASS':
        if (targetPlayer) {
          ...
          passTurn(targetIdx !== -1 ? targetIdx : null);
        } else {
      ...
      case 'CUT_WIRE':
        if (targetPlayer) {
          ...
          cutRandomWire(targetIdx !== -1 ? targetIdx : null);
        } else {
      ...
      case 'STEAL':
        if (targetPlayer) {
          ...
          if (victim && Array.isArray(victim.hand) && victim.hand.length > 0) {
